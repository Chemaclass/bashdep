...
```
# lib/.bashdep.lock
bashunit	https://github.com/TypedDevs/bashunit/releases/download/0.45.0/bashunit
...
|      no      |       n/a        |   n/a   | re-download |
|     yes      |       yes        |   yes   | re-download |

Bumping a release in the URL (`0.44.0` to `0.45.0`) re-downloads the affected entry only; nothing else is touched.

### Recommended workflow

Commit `.bashdep.lock` alongside your install script so collaborators get the same versions you do. To update one, edit its URL in your dependency list (or `.bashdep` file) and run install again. The lockfile updates automatically.
...
The annotation is stripped before download (`curl`/`wget` never see it) and can combine with the `@dev` suffix. If the checksum differs, or neither `shasum` nor `sha256sum` is available, the download fails. The file is removed and no lockfile entry is written. A present-but-empty or non-hex annotation (e.g. `#sha256=`) is rejected before download. Without the annotation, nothing is verified (the default).
...
Known boundaries of the flat, URL-driven model:
